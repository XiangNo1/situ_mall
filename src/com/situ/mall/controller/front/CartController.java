...
public class CartController {
	...
	@ResponseBody
	public CartItemVO AddDefaultCharsetFilter2(Integer productId, Integer amount, Model model, 
			HttpServletRequest request, HttpServletResponse response){
		System.out.println("productId:" + productId);
		System.out.println("amount:" + amount);
		//springmvc
		...
		//1.如果cookie有这个购物车对象，那就从cookie里面取出这个购物车对象
				...
				model.addAttribute("buyCartVO", buyCartVO);
				CartItemVO returncartItemVO = new CartItemVO();
		for (CartItemVO cartItemVO : items) {
			if (cartItemVO.getProduct().getId() == productId) {
				returncartItemVO.setAmount(cartItemVO.getAmount());
				returncartItemVO.setProduct(cartItemVO.getProduct());
			}
		}
		return returncartItemVO;
	}
	@RequestMapping(value="/cart")
	public String cart(){
		return "redirect:/cart/addCart.shtml";
